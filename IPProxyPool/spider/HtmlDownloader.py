            ...
            try:
                ...
                if 'r' in dir():
                    if r.text.find('Welcome To Zscaler Directory Authentication') > -1 or r.text.find(
                        ...
                        '<h1>Unauthorized ...</h1>') > -1 or r.text.find(
                        'Maximum number of open connections reached') > -1 or r.text.find(
                        unicode('净网卫士是江苏电信面向广大家长用户提供的一项增值服务')) > -1 or r.text.find(
                        '<head><title>404 Not Found</title></head>') > -1 or r.text.find(
                        '<title>404 - Not Found</title>') > -1 or r.text.find(
                        '<address>Apache/2.4.25 (Debian) Server a') > -1 or r.text.find(
                        'Too Many Users. Please contact Youngzsoft for support') > -1 or r.text.find(
                        unicode('<h1>帐号认证失败 ...</h1>')) > -1:
                        if ip:
            ...
            try:
                ...
                if 'r' in dir():
                    if r.text.find('Welcome To Zscaler Directory Authentication') > -1 or r.text.find(
                        ...
                        '<title>404 - Not Found</title>') > -1 or r.text.find(
                        '<address>Apache/2.4.25 (Debian) Server a') > -1 or r.text.find(
                        'Too Many Users. Please contact Youngzsoft for support') > -1 or r.text.find(unicode('<h1>帐号认证失败 ...</h1>')) > -1:
                        if ip:
                            requests.get('http://127.0.0.1:8000/delete?ip=%s' % ip)
                            print ('r.text.find, deeltip %s' % ip)
                        continue
